...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // Atualiza dados de perfil na navbar
...
// CADASTRO (CRIAR CONTA AUTH + BANCO FIRESTORE 'usuários')
...
if (registerForm) {
  registerForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // 2. Salva o documento na coleção 'usuários' (com acento) do Firestore
      await setDoc(doc(db, "usuarios", user.uid), {
        nome: regUsername || regEmail.split('@')[0],
        email: regEmail,
        nivel: "usuario",
