...
// LÓGICA DO CARROSSEL DE 3 SLIDES
...
function updateCarousel() {
  ...
  // Calcula os índices dos cards anterior, atual e próximo
  const prevIndex = (currentIndex - 1 + total) % total;
  const nextIndex = (currentIndex + 1) % total;

  // Atualiza imagens dos lados
  if (imgPrev) imgPrev.src = games[prevIndex].image;
  if (imgNext) imgNext.src = games[nextIndex].image;

  // Atualiza card principal (centro)
